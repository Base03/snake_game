...
import type { DrawableType } from "@rendering/drawableData";
import { createRenderSystem } from "@systems/RenderSystem";
import { registerRenderer, getRenderer } from "@rendering/renderers";

// Test-only helper: cast fake renderer type strings for the test registry.
// Production code uses real DrawableType values; tests need arbitrary strings.
const T = (s: string) => s as DrawableType;
...
describe("RenderSystem", () => {
  ...
  it("dispatches to registered renderer", () => {
    ...
    registerRenderer(T("__test_entity"), testRenderer);

    const world = new World();
    const id = world.spawn(
      cp("position", { x: 2, y: 3 }),
      cp("drawable", {
        type: T("__test_entity"),
        layer: 0,
        zIndex: 0,
        visible: true,
      }),
    ...
    expect(testRenderer).toHaveBeenCalledWith(
      ...
      48, // 3 * 16
      16,
      undefined, // data (no drawable.data set)
      id,
      world,
    );
  });
  ...
  it("skips invisible entities", () => {
    const ctx = mockCtx();
    const testRenderer = vi.fn();
    registerRenderer(T("__test_invisible"), testRenderer);

    const world = new World();
    world.spawn(
      cp("position", { x: 0, y: 0 }),
      cp("drawable", {
        type: T("__test_invisible"),
        layer: 0,
        zIndex: 0,
        visible: false,
      }),
  ...
  it("sorts by layer then zIndex", () => {
    const ctx = mockCtx();
    const order: string[] = [];
    registerRenderer(T("__test_order_a"), () => order.push("a"));
    registerRenderer(T("__test_order_b"), () => order.push("b"));
    registerRenderer(T("__test_order_c"), () => order.push("c"));
